...
Golden hour uses matched afternoon/dusk paintings with staggered spatial masks. Water displacement must sample the current composited painting, never the daytime source alone. Keep compositing bounded and cached, retain original image coordinates through cover crops, load the extra state only for the meadow, and preserve visible retry if it fails. Other places still use subtle procedural evening grades.
