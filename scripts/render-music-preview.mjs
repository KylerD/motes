...
try {
  ...
  const result=await page.evaluate(async({seconds,seed,mood,index})=>{
    const {renderPreview}=await import('/src/music/audio.ts');
    const {buffer,track}=await renderPreview({seconds,seed,mood,index,mode:'beats'});
    ...
    return {wav:btoa(binary),report:{seconds,sampleRate:buffer.sampleRate,peak,rms:Math.sqrt(power/(buffer.length*channels.length)),clipped,blockRms,title:track.title,bpm:track.bpm,key:track.key,voice:track.voice,events:track.events.length}};
  },{seconds,seed,mood,index});
